...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="navtable">
  <tr>
    <td height="31"><ul id="MenuBar1" class="MenuBarHorizontal" >
     <c:if test="${login.person.type=='BOTH'}"> <li><a href="<c:url value="FamilyPhysicianManagement.htm"/>" onclick="testFunction()"><fmt:message key="patient.management"/></a></li>
     </c:if>
       <li><a id="sepReff" href="<c:url value="SpecalistManagement.htm"/>" onclick="testFunction()"><fmt:message key="referral.patients"/></a></li>
      <c:if test="${login.person.type=='SPECALIST'}">
			<li><a id="sepMenuStatistics" href="<c:url value="SpecalistStatistics.htm"/>" onclick="testFunction()"><fmt:message key="sepMenu.Statistics"/></a></li>
            </c:if> 
      <c:if test="${login.person.type=='BOTH'}">
        ...
        <c:if test="${login.person.type=='BOTH'}">
            <li><a id="sepMenuStatistics" href="#"><fmt:message key="fpMenu.Statistics"/>&nbsp;</a>
				<ul>
       ...
       <c:if test="${login.person.type=='SPECALIST'}">
       <li><a id="myNetworkNew" href="<c:url value="MyNetwork.htm"/>" onclick="testFunction()"><fmt:message key="network.doctors"/></a></li>
       </c:if>
    </ul></td>
  </tr>
</table>
