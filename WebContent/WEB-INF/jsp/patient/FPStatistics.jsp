...
<script type="text/javascript" src="<fmt:message key="project.context"/>/applicationScript/FPStatistics.js"></script> 
...
<body onload="init('<c:out value="${login.loginId}"/>','<fmt:message key="select_sep"/>','<fmt:message key="select_lab"/>','<fmt:message key="patients_registered"/>','<fmt:message key="pat_from"/>','<fmt:message key="pat_to"/>','<fmt:message key="patients_referred"/>','<fmt:message key="patients_ordered"/>','<fmt:message key="patients_ordered_referred1"/>','<fmt:message key="patients_ordered_referred2"/>','<fmt:message key="pat_is"/>'); hide('successMessage'); hide('errorMessage');">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="pageHeadingL2">
    <tr>
        <td align="center">Date <fmt:message key="pat_from"/>&nbsp;<input name='fromDateS' id="fromDate"  size='11' maxlength="11" OnKeyPress="captureReturnKey(event)" onclick='scwShow(this,event);' readonly>&nbsp;<fmt:message key="pat_to"/>&nbsp;<input name='toDateS' id="toDate"  size='11' maxlength="11" OnKeyPress="captureReturnKey(event)"  onclick='scwShow(this,event);' readonly>
         <select name="spceialistN" id="spceialist" onChange="changeTypeSep('<fmt:message key="enteredDate_Invalid"/>','<fmt:message key="fromTodate_Invalid"/>','<c:out value="${login.loginId}"/>')"></select> <select name="labN" id="lab" onChange="changeTypeLab('<fmt:message key="enteredDate_Invalid"/>','<fmt:message key="fromTodate_Invalid"/>','<c:out value="${login.loginId}"/>')"></select>
          <input type="button" value="Ok" name="okB" onclick="clickOK('<fmt:message key="enteredDate_Invalid"/>','<fmt:message key="fromTodate_Invalid"/>','<c:out value="${login.loginId}"/>','<fmt:message key="select_sep"/>','<fmt:message key="select_lab"/>','<fmt:message key="patients_registered"/>','<fmt:message key="pat_from"/>','<fmt:message key="pat_to"/>','<fmt:message key="patients_referred"/>');"> <input type="button" value="?" name="commHelp">
       </td>
   </tr>
</table>
<br>
...
<table width="100%" border="0" align="center" class="viewtable">
  ...
  <tbody id="patientbody">
    <tr id="pattern" style="display:none;">
      ...
      <td width="25%" align="center"><input id="newCase" type="button" class="viewemr" title="View EMR" value="" onClick="testFunction(); emrViewClicked(this.id,'<c:out value="${login.loginId}"/>');"/>&nbsp;</td>
    </tr>
   </tbody>
</table>
<br>
